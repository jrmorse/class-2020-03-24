...
```{r Learn Data}
# these are ways to get to know the data
glimpse(train)
...
```{r Average Income}
# Either one of these work, but the second version is a bit better because it is more flexible
...
```{r Bootstrap}
# Use rep sample n. The total number of people both treated and not treated is 115. We then group by replicate. From there we can use the summarize function to find the mean income for each replicate.
...
```{r}
# Call in what we had done earlier. We need the averages for each replicate
train %>% 
rep_sample_n(115, replace = TRUE, reps = 1000) %>% 
  group_by(replicate) %>% 
  summarize(mean_income = mean(income)) %>% 
# we need to pull out the variable we're interested in 
   pull(mean_income) %>% 
# we are interested in the 50% ci Do the quantile method to pull desired values
  quantile(c(0.25,0.75))

```
I have 95% confidence that the true mean is between this value
...
```{r}
train %>% 
# First we group by treatment vs not treated
  group_by(treatment) %>% 
# find the average income for each group
  summarize(mean_income = mean(income)) %>% 
# Pivot wider but not sure why
  pivot_wider(
    names_from= treatment,
    values_from = mean_income
  ) %>% 
  # Make a new column to make it obvious what is the difference
...
rep_sample_n(115, replace = TRUE, reps = 1000) %>% 
  ...
  mutate(diff = Treated - Control) %>% 
# pull the diff column because these are the values we're interested in
  pull(diff) %>% 
  quantile(c(.025, .975))
```
...
prompt9 <- train %>%
  ...
  mutate(att_chg_difference = Treated - Control) %>%
  select(att_chg_difference, everything()) %>%
  pull(att_chg_difference) %>%
